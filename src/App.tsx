...
class App extends Component<object, IAppState> {
	...
	handleNewMessages = (snap: firebase.firestore.QuerySnapshot) => {
		this.setState({ messages: [].concat(this.state.messages, snap.docChanges().map(x => x.doc.data())) });
	};
	componentDidMount() {
		this.cleanup = firebase.firestore().collection('chatroom').orderBy("timestamp", "asc").onSnapshot(this.handleNewMessages);
	}
	...
	handleClick = e => {
		firebase.firestore().collection('nicknames').add({
